...
const authenticClient = axios.create({
  ...
  headers: { 'Authorization': `Token ${process.env.AUTHENTIC_WALLET_TOKEN}` }
});

async function authentic_createReceipt() {
  const receipt_response = await authenticClient.post('/api/receipts/', defaultReceipt,
    {
...
app.get('/api/i-want-nft', async (req, res) => {
  ...
  try {
    ...
    return res.redirect(`${process.env.AUTHENTIC_WALLET_API_URI}rpc/session.execute/?session_token=${session}`)
  } catch (error) {
    console.error({
      error: error
    })
